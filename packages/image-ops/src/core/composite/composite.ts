/**
 * Pure image compositing functions for the core layer.
 *
 * This module contains pixel-level compositing logic extracted from the original
 * composite.ts. All functions are pure - no canvas, no platform APIs.
 *
 * Architecture:
 * - core/composite/composite.ts: Pure algorithm, no platform dependencies
 * - core/blend-modes.ts: Blend mode formulas
 * - core/porter-duff.ts: Porter-Duff compositing
 * - core/alpha-format.ts: Alpha format detection
 *
 * @example
 * import { compositeImages } from '@prism/image-ops/core/composite';
 */
  ...
  for (let cy = 0; cy < clippedH; cy++) {
    for (let cx = 0; cx < clippedW; cx++) {
      ...
      if (baseFormat === 'premultiplied') {
        ...
        basePx = [result[dstIdx], result[dstIdx + 1], result[dstIdx + 2], result[dstIdx + 3]]
        ovPx = [
