...
export const transformExecutor: NodeExecutor = async (inputs, params) => {
  ...
  if (scaleX !== 1 || scaleY !== 1) {
    const targetWidth = Math.round(
      (cropWidth > 0 ? (cropWidth > 0 ? cropWidth : imageData.width) : imageData.width) *
        Math.abs(scaleX),
    )
    const targetHeight = Math.round(
      (cropHeight > 0 ? cropHeight : imageData.height) * Math.abs(scaleY),
    )
