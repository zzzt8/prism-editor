...
import { bufferToImageData } from './sharp-utils';
...
export const loadImageExecutor: NodeExecutor = async (
  ...
  switch (source.type) {
    case 'url':
      ({ buffer, mimeType } = await loadFromUrl(source.value));
      break;
    case 'path':
      ({ buffer, mimeType } = await loadFromPath(source.value));
      break;
    case 'buffer':
      buffer = source.value;
      mimeType = inferMimeType(buffer);
      break;
  }
