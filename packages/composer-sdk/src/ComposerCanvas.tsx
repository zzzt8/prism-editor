...
import { applyMaskToImageData } from './utils/imageMaskUtils';

// Re-export for backwards compatibility (Facade).
export { imageToImageData } from './utils/imageToImageData';
export { applyMaskToImageData } from './utils/imageMaskUtils';
...
export const ComposerCanvas: React.FC<ComposerSDKProps> = ({
  ...
  const renderComposite = useCallback(async () => {
    ...
    // Apply active mask to the composite result if present
    if (activeMask) {
      try {
        const compositeImageData = ctx.getImageData(0, 0, width, height);
        const maskedData = applyMaskToImageData(compositeImageData, activeMask, width, height);
        ctx.putImageData(maskedData, 0, 0);
      } catch (e) {
        console.warn('Failed to apply mask:', e);
      }
    }
  }, [layers, loadedImages, width, height, backgroundColor, activeMask]);
