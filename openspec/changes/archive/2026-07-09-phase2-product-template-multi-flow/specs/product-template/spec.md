# Spec: ProductTemplate 管理

> OpenSpec ADDED delta — Phase 2 引入
> 归档时合并到 `openspec/specs/product-template/spec.md`
...
| 依赖 | 说明 |
|------|------|
| Phase 0 Prisma schema | ProductTemplate / Workflow / Asset 三表外键结构 |
| Phase 1 image-ops 三层拆分 | engine 层不需修改 |
...
- ~~Composer SDK 创建 template 的接口~~（Phase 3）
- ~~WorkflowVersion 独立表~~（Phase 3+）
- ~~批量操作 endpoint~~（Phase 4 mall 集成）