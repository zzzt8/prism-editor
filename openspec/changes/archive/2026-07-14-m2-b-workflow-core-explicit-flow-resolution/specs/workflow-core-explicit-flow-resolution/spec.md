...
> **Capability**: workflow-core-explicit-flow-resolution
> **Source change**: m2-b-workflow-core-explicit-flow-resolution
> **Layer**: engine (`@prism/workflow-core`)

## Purpose

In `@prism/workflow-core`, implement deterministic Flow resolution and explicit output collection: locate a Flow by exact `(templateId, templateVersion, flowKey)`; collect `RenderResult.outputs` in `Flow.explicitOutputs` declaration order; eliminate `findFirst` / `Object.keys(...).pop()` implicit selection.
