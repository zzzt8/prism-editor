...
> **Capability**: server-deterministic-render-entry
> **Source change**: m2-c-server-deterministic-render-entry
> **Layer**: server (`server/`)

## Purpose

In `server`, implement the deterministic production render entry: locate a Flow by exact `TemplateVersion + flowKey`; eliminate `prisma.workflow.findFirst` and `Object.keys(results).pop()` violations.
