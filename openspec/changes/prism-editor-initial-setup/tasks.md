...
- [ ] 8.1 实现工作流保存功能，**通过 Storage Adapter 接口**
- [ ] 8.2 实现工作流加载功能，**通过 Storage Adapter 接口**
- [ ] 8.3 实现工作流导出 JSON
- [ ] 8.4 实现工作流导入 JSON

## 8.x. Storage Adapter 存储适配器层

- [ ] 8.x.1 定义 `StorageAdapter` 接口
  ```typescript
  ...
  ```
- [ ] 8.x.2 实现 `LocalStorageAdapter`（MVP 阶段使用 localStorage）
- [ ] 8.x.3 实现 `JsonFileAdapter`（导出/导入 JSON 文件）
- [ ] 8.x.4 在 workflowStore 中集成 Storage Adapter
- [ ] 8.x.5 预留云端存储扩展点（未来可接入 Notion Database、Supabase、Firebase 等）
...
- [ ] 14.1 编写 LoadImage → Export 完整链路测试
- [ ] 14.2 编写带 Mask 的合成流程测试
- [ ] 14.3 编写发布-运行完整流程测试

## 15. 项目整理

- [ ] 15.1 更新 README.md 文档
- [ ] 15.2 配置 .gitignore
- [ ] 15.3 初始化 Git 仓库并提交
